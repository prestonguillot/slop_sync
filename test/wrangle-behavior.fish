...
# ─── Dry-run is hermetic ─────────────────────────────────────────────────
# --dry-run must never create a last-wrangle stamp.

set -l fake_home (mktemp -d)
env HOME=$fake_home $wrangle --dry-run >/dev/null 2>&1
test -f $fake_home/.cache/dotfiles/last-wrangle
@test "--dry-run does NOT create last-wrangle stamp" $status -ne 0
rm -rf $fake_home
